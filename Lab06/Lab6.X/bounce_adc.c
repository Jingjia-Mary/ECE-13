/* 
 * This file contains the implement functions for bounce_adc.
 * File:   bounce_adc.c
 * 
 * Author: Mary Xu
 *
...
int main(void) {
     ...
     **************************************************************************************************/
    printf("Welcome to Mary's lab6 part3 (bounce_adc).  Compiled on %s %s.\n", __TIME__, __DATE__);

    OledInit();
    Adc1.event = TRUE;
    char Draw[35];
    while (1) {
        if (Adc1.event == TRUE) {// calculations
            int value = Adc1.voltage / 1023.0 * 100.0;
            sprintf(Draw, "Potentiometer value:\n %d \n %d%%", Adc1.voltage, value);
            
            OledClear(OLED_COLOR_BLACK); // set color to black
            OledDrawString(Draw);
            OledUpdate();
            Adc1.event = FALSE;
        }
...
void __ISR(_ADC_VECTOR, ipl2auto) AdcHandler(void) {
    ...
    IFS1bits.AD1IF = 0;
    int sum = ADC1BUF1 + ADC1BUF2 + ADC1BUF3 + ADC1BUF4 + ADC1BUF5 + ADC1BUF6 + ADC1BUF7 + ADC1BUF0;
    int16_t average = sum / 8;
    if (abs(average - Adc1.voltage) >= NUM) {
        Adc1.event = TRUE;
